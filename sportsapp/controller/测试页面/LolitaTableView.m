...
//        NSLog(@"--111--y-- = %lf -stayPosition-- = %lf",y,stayPosition);

        if ([self.delegate_StayPosition respondsToSelector:@selector(lolitaTableViewHeightForStayPosition:)]) {
            stayPosition = [self.delegate_StayPosition lolitaTableViewHeightForStayPosition:self]-Height_NavBar;  // 获取到停留的位置
            //NSLog(@"--222--y-- = %lf -stayPosition-- = %lf",y,stayPosition);
        }

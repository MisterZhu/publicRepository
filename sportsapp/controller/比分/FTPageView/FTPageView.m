...
#define Height_btn  49.0

#define LINEBOTTOM_HEIGHT 2
#define TOPBOTTOMLINEBOTTOM_HEIGHT 0
#define SELECTED_COLOR [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0]
#define UNSELECTED_COLOR [UIColor colorWithRed:140/255.0 green:158/255.0 blue:182/255.0 alpha:1.0]
#define TPOTABBOTTOMLINE_COLOR [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0]

#import "FTPageView.h"
#import <objc/runtime.h>

@interface FTPageView () <UIScrollViewDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame withTitles:(NSArray *)titles withViewControllers:(NSArray *)controllers withParameters:(NSArray *)parameters{
    ...
    if (self) {
        ...
        _selfFrame             = frame;
        _selectedColor         = UIColorFromRGB(kColorRed, 1.0);
        _unselectedColor       = UIColorFromRGB(kColorGSecondary, 1.0);
...
//    _font = _font?_font:[UIFont fontWithName:@"PingFangSC-Medium" size:16];
...
- (UIView *)topTabView{
    if (!_topTabView){
        CGRect frame = CGRectMake(0, 0, _selfFrame.size.width, Height_btn);
        _topTabView  = [[UIView alloc] initWithFrame:frame];
        _topTabView.backgroundColor = UIColorFromRGB(kColorWhite, 1.0);
        if (_isShowTopTabBottomLine) {
            UIView *topTabBottomLine = [UIView new];
            topTabBottomLine.frame = CGRectMake(0, Height_TabBar - TOPBOTTOMLINEBOTTOM_HEIGHT, _selfFrame.size.width, TOPBOTTOMLINEBOTTOM_HEIGHT);
...
- (UIScrollView *)topTabScrollView{
    if (!_topTabScrollView){
        ...
        CGRect frame = CGRectMake(leftWidth, _topTabView.frame.origin.y, _topTabScrollViewWidth, Height_btn);
        ...
        if (self.pageViewStyle == FTPageViewStyleB) {
        ...
        for (NSInteger i=0; i<_titles.count; i++) {
            ...
            CGRect buttonFrame = CGRectMake(x-width/2, 0, width, Height_btn);
        ...
        if (_isShowTopTabBottomLine) {
            UIView *topTabBottomLine = [UIView new];
            topTabBottomLine.frame = CGRectMake(-totalWidth, Height_btn - TOPBOTTOMLINEBOTTOM_HEIGHT, totalWidth*3, TOPBOTTOMLINEBOTTOM_HEIGHT);
            topTabBottomLine.backgroundColor = _topTabBottomLineColor;
            [_topTabScrollView addSubview:topTabBottomLine];
        }
        
        _lineBottom = [[UIView alloc] initWithFrame:CGRectMake(0, Height_btn - LINEBOTTOM_HEIGHT-4,[_titleSizeArray[self.defaultSubscript] CGSizeValue].width/3, LINEBOTTOM_HEIGHT)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSLog(@"scrollViewDidEndScrollingAnimation = %ld",self.currentPage);
    
    WS(weakSelf);
    if (weakSelf.clickTitleBlock) {
        weakSelf.clickTitleBlock(weakSelf.currentPage);
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    _lineBottom.bounds = CGRectMake(0, 0-4, [self getTitleWidth:scrollView.contentOffset.x]/3, LINEBOTTOM_HEIGHT);
...
- (void)clickAction:(UIButton *)button {
    ...
    WS(weakSelf);
    if (weakSelf.clickTitleBlock) {
        weakSelf.clickTitleBlock(button.tag);
    }
    NSLog(@"clickactionbtn.tag = %ld",button.tag);
}
